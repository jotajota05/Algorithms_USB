class ArbolMin extends ArbolDeJuego {
	...
	public ArbolMin (Tablero t,int k, ValCasilla jug) {
		this.t = t;
		this.k = k;
		this.jug = jug;
				...
				while (j<3){
					if (this.t.tab[i][j].esVacia()){
						...
						if (!hayTableroEquivalente){
							...
							ArbolDeJuego hijo= new ArbolMax(tAuxiliar1,this.k-1,oponente);
							hijo.expandir();
							this.hijos = this.hijos.inserta(tAuxiliar1.beneficio(this.jug),hijo);
	 ...
	 *PRE: recibe un objeto del tipo ArbolDeJuego
	 *POST: retorna el tablero cuyo rango es el menor de una lista de tableros que representan las 
	...
	public Tablero decisionSobreTablero(){
		...
		while (Hijos instanceof Cons){
			...
			int maximoRangoSubHijos = -9;
			while (subHijos instanceof Cons){
				maximoRangoSubHijos = subHijos.obtRango();
				subHijos = subHijos.obtLista();
			}
			opciones = opciones.insertaEnOrden(maximoRangoSubHijos, Hijos.obtObjeto().obtTablero());
